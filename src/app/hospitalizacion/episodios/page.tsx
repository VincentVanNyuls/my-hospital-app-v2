...
export default function EpisodiosPage() {
  ...
  const cargarEpisodios = async () => {
    try {
      ...
      setEpisodios(todosEpisodios || []); // ✅ Asegurar que siempre sea array
    } catch (error) {
      console.error('Error cargando episodios:', error);
      setEpisodios([]); // ✅ Fallback a array vacío
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtrarEpisodios = () => {
    // ✅ CORREGIDO: Verificar que episodios es array
    if (!episodios || !Array.isArray(episodios)) {
      setEpisodiosFiltrados([]);
      return;
    }

    let filtrados = [...episodios]; // ✅ Crear copia
  ...
  // ✅ CORREGIDO: Con verificación de array
